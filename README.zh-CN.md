...
| 形状 | 实心圆 = 假眼 · 空心圈 = 真眼 · **叉 = 被排掉了**（停留 20 秒，期间逐渐变淡） |
...
> 不想按 Alt 的话，有个「**始终显示**」，勾上之后对局中一直显示。
> 三处都能拨：设置卡片、通知区菜单、或者全局热键 **`Ctrl+Alt+F11`**——
> 对局中途要开关就用热键，进设置会抢焦点、没法继续玩。
> 它只是去掉"按住 Alt"这个条件——**回到主菜单照样消失**，不会杵在桌面上。
...
**左键单击通知区图标**（或右键 →「编辑面板 (Ctrl+Alt+F10)」，或直接按热键）
进入编辑态：四个块
...
| **显示项** — 九个格子各自的开关 | **面板** — 始终显示、语言、缩放、透明度、底板、地图大小、重置 |
...
| `records/` | 对局录制（默认关闭）。**一局一个文件**，名字里带对局号：`raw_<秒>_m<matchid>.jsonl.gz` |
