...
| | |
|---|---|
| `constants/` | Timing tables, item prices, language packs — edit the JSON and restart |
...
Every constant lives outside the binary in `constants/*.json` — timing tables (one each
for normal and turbo), item prices, language packs. A game patch means editing data, not
code; the predecessor project died precisely of constants baked into the source.

`constants/patch.json` records which Dota version these constants are aligned to:

```json
{ "dota": "7.41e", "synced": "2026-09-09" }
```

**Item prices are editable too.** If net worth is off by exactly one item's cost, edit
`constants/item_prices.json` in the config directory and restart — what is on disk wins
over the copy built into the binary. (The next time the app moves to a new Dota version
that table is replaced wholesale and your edits are not kept: prices change with the
patch, and keeping an old value would only mask the correct one. Timing tables are
untouched.)

The price table is generated by `python tools/sync_constants.py` from **the game's own
files**: costs from `scripts/npc/items.txt` (`ItemCost`), item ids from
...
the local Dota VPK, updates the version in `constants/patch.json`, and prints two
