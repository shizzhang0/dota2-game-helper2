...
| | |
|---|---|
| `constants/` | Timing tables and language packs — edit the JSON and restart (item prices are not here, see below) |
...
- Item prices: local constants (snapshotted from the game's own data files) — **no
  network access at runtime, and no reading of game files either**

Constants live outside the binary in `constants/*.json` — timing tables (one each for
normal and turbo) and language packs. A game patch means editing data, not code; the
predecessor project died precisely of constants baked into the source.

**Which Dota version these constants are aligned to lives in `constants/patch.json`** —
the same value as the badge at the top:

```json
{ "dota": "7.41e", "synced": "2026-09-09" }
```

The price table is the one exception: it is **not** placed in the config directory, only
compiled into the binary. It is generated by `python tools/sync_constants.py` from **the
game's own files**: costs from `scripts/npc/items.txt` (`ItemCost`), item ids from
`npc_ability_ids.txt`. That happens on the development machine only — the shipped exe
contains none of that parsing and never goes looking through the Dota install for
prices. What you get is a fixed constant, so when a number is off we can tell which
version of the table you were on.

So prices are not editable, and in the days between a patch landing and a release here
you have to wait. What that buys is that an old table on disk can never silently mask
the correct one — which is far harder to notice than prices being a few days late.
Timing tables are not affected and stay editable.
...
After a Dota patch, run `sync_constants.py` once: it regenerates the price table from
the local Dota VPK, updates the version in `constants/patch.json` and the version badge
at the top of both READMEs, and prints two
things — how prices changed since the last version, and the entries in this patch's
notes that mention runes, Roshan, towers, glyph, buyback, Lotus or neutral stacking.
Prices are automatic; those entries want a human read, to decide whether
`normal.json` / `turbo.json` / `towers.json` need touching — rune intervals and the like
are not in the game's item table.
