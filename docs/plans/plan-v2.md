...
> **历史归档。** 这是当初逐任务执行时用的实施计划，保留为过程记录，不再维护。
> 当前的设计结论请看 [../design/](../design/) 下按主题组织的文档；
> 两者冲突时以 design/ 为准。

> 逐任务执行，步骤用 checkbox（`- [ ]`）跟踪进度。

**Goal:** 实现：眼位小地图、托盘与编辑态、文件日志、对局录制。
...
详见 [design/overlay.md](../design/overlay.md)。
...
- **返工记录**：Task 4/6 的「独立设置窗口 + 托盘四项」上手后推翻，由 Task 9 合并为单一编辑态；理由（双置顶必互遮 + 面板平时藏着导致盲调）已写入 design/overlay.md。Task 4 Step 1 的 settings.js 是唯一存活至今的产物。
