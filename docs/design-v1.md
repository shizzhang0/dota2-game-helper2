...
- 价格表来自 OpenDota `constants/items`（Rust 侧启动拉取 + 本地缓存 + 内嵌快照三层兜底），
  ...
  随版本自动更新，查不到才退回常数表。
- **OpenDota 的价格会落后于游戏版本**，且它是我们能拿到的最好的源——
  游戏自己的价格在 VPK 包里，要自己写解析且格式随版本变，不值得。
  因此加一张本地覆盖表 `constants/item_price_overrides.json`（`物品名: 实际价格`，
  下划线开头的键当注释跳过），在 `get_item_prices` 返回前套用。
  与"常数外置"同一套路子：改 JSON 重启生效，不必重新编译。
  上游修好后删掉对应行即可——套用时若发现覆盖值与上游已经一致，会写一条日志提示可以删。

  已知需要覆盖的（2026-09-07 对照回放官方"财产总和"实测）：

  | 物品 | OpenDota | 游戏实际 |
  |---|---|---|
  | 龙心 `heart` | 5100 | **5200** |

  同一局里辉耀 4700 / 冰眼 5900 / 法师克星 3100 / 相位鞋 1400 / 影刃 3250 /
  银月 5700 / 魔晶 1400 / 魔杖 460 / 掠夺者 2800 均已验证正确，
  所以这是个例，不是整张表都偏。
...
- **快速模式的符间隔已实测确认与正常模式一致**（2026-09-02 进游戏核对，无明显偏差），
  两张表数值相同。此前 dump 校准（n=13 次赏金符拾取）无法区分 180s 与 240s，以实测为准。
