...
- **炼金送的神杖不计**：炼金术士可以把神杖直接送给队友，受赠方一分钱没花却同样拿到
  `modifier_item_ultimate_scepter_consumed`，而 permanent_buffs 里**没有 purchaser 字段**，
  无法从 buff 本身判断归属。
  判据是物品栏历史：自己买的神杖会先在包里待着（实测一局待了 827 秒才吞），
  炼金送的则从不进受赠方的物品栏。所以只在"曾见过该物品且 `purchaser` 是自己"时才计。
  月之碎片同理（队友买了递给你吃的情况）。
  兜底：程序中途启动、第一眼 buff 就已存在时无从判断，按常见情况（自己买的）计入。
